export class ExtendedTestModel extends StandardItemModel {

    static defineSchema() {
        const fields = foundry.data.fields;
        let schema = super.defineSchema();
        schema.acquiredSL = new fields.NumberField({ initial: 0 });
        schema.totalSL = new fields.NumberField({ initial: 0, min: 0 });
        schema.timeframe = new fields.StringField({ initial: "" });
        schema.tests = new fields.ArrayField(new fields.SchemaField({
            ...
            required: new fields.BooleanField({ initial: true }),
            done: new fields.BooleanField({ initial: false }),
            timeframe: new fields.StringField({ initial: "" }),
            countNegativeSL: new fields.BooleanField({ initial: false }),
            criticalDoubleSL: new fields.BooleanField({ initial: false }),
            fumbleDoubleSL: new fields.BooleanField({ initial: false }),
    ...
    async summaryData() {
        ...
        data.totalSL = Number(this.totalSL || 0);
        ...
        data.tests = (this.tests || []).map((test, index) => {
            ...
            const required = Boolean(test.required);
            const done = Boolean(test.done);
            const timeframe = test.timeframe || "";
            const countNegativeSL = Boolean(test.countNegativeSL);
            const criticalDoubleSL = Boolean(test.criticalDoubleSL);
            const fumbleDoubleSL = Boolean(test.fumbleDoubleSL);
            ...
            return {
                ...
                showDone: done,
                showTimeframe: Boolean(timeframe),
                showCountNegativeSL: countNegativeSL,
                showCriticalDoubleSL: criticalDoubleSL,
                showFumbleDoubleSL: fumbleDoubleSL,
                assignee: {
                    uuid: assignee.uuid || "",
                    name: assignee.name || "",
                    img: assignee.img || ""
